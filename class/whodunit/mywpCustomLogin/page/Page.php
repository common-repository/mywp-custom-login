...
abstract class Page{
	...
	function __construct( $parent_page_slug = null, $page_slug = null, $page_template = null, $page_cap = null, $page_title = null, $page_menu_title = null, $page_menu_pos = null, $page_icon = null ){
		if( ! empty( $parent_page_slug ) && is_string( $parent_page_slug ) ){
			$this->parent_page_slug = ( in_array( $parent_page_slug, [ 'options-general.php' ] ) ) ? $parent_page_slug : sanitize_title( $parent_page_slug );
		}
	...
	function admin_footer_text( $text ){
		...
		if( $this->page_slug === $hook_suffix ){
			return '<span id="footer-thankyou">'.__( 'Thank you for using our product :)', 'mywp-custom-login').'</span>';
		}
		return $text;
	}
	...
	function admin_footer_update( $text ){
		global $hook_suffix;
		if( $this->page_slug === $hook_suffix ){
			return MyWPCustomLogin::get_instance()->get_version( 'short' );
		}
		return $text;
	}
	...
	public function load_template( $attributes = [], $echo = false ){
		...
		$file = MyWPCustomLogin::get_instance()->get_dir().'views/pages/'.$this->page_template.'.php';
		if( file_exists( $file ) ){
	 ...
	 * TODO : comment
	 */
	public function load_part( $part = '', $echo = false ){
		if( false === $echo ){ ob_start(); }
		$file = MyWPCustomLogin::get_instance()->get_dir().'views/parts/'.$part.'.php';
		if( file_exists( $file ) ){
			include( $file );
		}
		if( false === $echo ){ return ob_get_clean(); }
